...
class Dataset:
    ...
    def _process_for_double_LSTM(self):
        if self.cfg.DATASETS.FEATURES_TYPE == 1:
            f_p1 = pd.read_csv(self.cfg.DATASETS.FEATURES_PER1_D)
            f_p2 = pd.read_csv(self.cfg.DATASETS.FEATURES_PER2_D)
        elif self.cfg.DATASETS.FEATURES_TYPE == 2:
            f_p1 = pd.read_csv(self.cfg.DATASETS.FEATURES2_PER1_D)
            f_p2 = pd.read_csv(self.cfg.DATASETS.FEATURES2_PER2_D)
        elif self.cfg.DATASETS.FEATURES_TYPE == 3:
            f_p1 = pd.read_csv(self.cfg.DATASETS.FEATURES3_PER1_D)
            f_p2 = pd.read_csv(self.cfg.DATASETS.FEATURES3_PER2_D)
    ...
    def _process_for_triple_LSTM(self):
        ...
        x_train_dist = pd.DataFrame(self.imputer.transform(x_train_dist), columns=x_train_dist.columns, index=x_train_dist.index)
        x_train_dist = pd.DataFrame(self.scaler.transform(x_train_dist), columns=x_train_dist.columns, index=x_train_dist.index)
        x_train_dist = pd.concat([x_train_dist, full_data_train_dist[["batch"]]], axis=1)

        x_test_per1 = full_data_test_per1.drop(["class", "batch", "camera_id", "subject_id", "R_id"], axis=1)
        x_test_per1 = pd.DataFrame(self.imputer.transform(x_test_per1), columns=x_test_per1.columns, index=x_test_per1.index)
        x_test_per1 = pd.DataFrame(self.scaler.transform(x_test_per1), columns=x_test_per1.columns, index=x_test_per1.index)
        x_test_per1 = pd.concat([x_test_per1, full_data_test_per1[["batch"]]], axis=1)

        x_test_per2 = full_data_test_per2.drop(["class", "batch", "camera_id", "subject_id", "R_id"], axis=1)
        x_test_per2 = pd.DataFrame(self.imputer.transform(x_test_per2), columns=x_test_per2.columns, index=x_test_per2.index)
        x_test_per2 = pd.DataFrame(self.scaler.transform(x_test_per2), columns=x_test_per2.columns, index=x_test_per2.index)
        x_test_per2 = pd.concat([x_test_per2, full_data_test_per2[["batch"]]], axis=1)

        x_test_dist = full_data_test_dist.drop(["class", "batch", "camera_id", "subject_id", "R_id"], axis=1)
        x_test_dist = pd.DataFrame(self.imputer.transform(x_test_dist), columns=x_test_dist.columns, index=x_test_dist.index)
        x_test_dist = pd.DataFrame(self.scaler.transform(x_test_dist), columns=x_test_dist.columns, index=x_test_dist.index)
    ...
    def create_test_train_sets(self):
        ...
        if self.cfg.MODEL.ARCH == 'triple':
            ...
            X_train_seq_per1, y_train_seq, X_train_dist_seq = create_spaced_sequences(x_train_per1, y_train, x_train_dist, self.cfg.SEQUENCE.LIN_SIZE, train_batches)
            X_train_seq_per2, _ , _ = create_spaced_sequences(x_train_per2, y_train, x_train_dist, self.cfg.SEQUENCE.LIN_SIZE, train_batches)
            test_batches = [batch for batch in all_batches if batch not in train_batches]
            X_test_seq_per1, y_test_seq, X_test_dist_seq = create_spaced_sequences(x_test_per1, y_test, x_test_dist, self.cfg.SEQUENCE.LIN_SIZE, test_batches)
            X_test_seq_per2, _ , _ = create_spaced_sequences(x_test_per2, y_test, x_test_dist, self.cfg.SEQUENCE.LIN_SIZE, test_batches)
